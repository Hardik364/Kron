...
#[tracing::instrument(skip(state), fields(user_id = %user.user_id, tenant_id = %user.tenant_id))]
    ...
    for (idx, document) in body.yaml.split("\n---\n").enumerate() {
        ...
        match parsed {
            Ok(yaml_val) => {
                ...
                let rule_id = uuid::Uuid::new_v4().to_string();
                let config = serde_json::to_value(&yaml_val).unwrap_or(serde_json::Value::Null);
