...
export default function AlertsPage() {
    ...
    const { state, fetchAlerts, acknowledgeAlert, markFalsePositive, selectAlert, prependAlert } = useAlerts();
    const [statusFilter, setStatusFilter] = createSignal('open');
    ...
    createEffect(() => {
        if (!isAuthenticated())
            return;
        const ws = apiClient.createAlertWebSocket((alert) => prependAlert(alert), () => {
        ...
        const handler = (e) => {
            ...
            switch (e.key) {
                ...
                case 'F': {
                    if (state.selectedId) {
                        e.preventDefault();
                        void handleFalsePositive();
                    }
                    break;
                }
                case ' ': {
                    e.preventDefault();
                    if (!state.selectedId && alerts[0])
    ...
    const selectedAlert = () => state.alerts.find((a) => a.alert_id === state.selectedId);
    ...
    return (<Layout title="Alert Queue">
      <div style={{
            display: 'flex',
            gap: '0',
        ...
        <div style={{
          ...
          <div style={{ 'overflow-y': 'auto', flex: '1' }}>
            <Show when={!state.isLoading} fallback={<div style={{ padding: '16px', display: 'flex', 'flex-direction': 'column', gap: '10px' }}>
                  <For each={Array.from({ length: 8 })}>
                    {() => <LoadingSkeleton height={52} radius={6}/>}
                  </For>
                </div>}>
              <Show when={state.error === null} fallback={<div style={{ padding: '16px', color: 'var(--danger)', 'font-size': '13px' }}>
                    {state.error}
                  </div>}>
                <Show when={state.alerts.length > 0} fallback={<div style={{
                padding: '48px 16px',
                'text-align': 'center',
                color: 'var(--text-muted)',
                'font-size': '13px',
        ...
        <div style={{ flex: '1', 'overflow-y': 'auto', padding: '24px' }}>
          <Show when={selectedAlert() !== undefined} fallback={<div style={{
                display: 'flex',
                'flex-direction': 'column',
                'align-items': 'center',
            ...
            if (!alert)
                return null;
            return (<div style={{ display: 'flex', 'flex-direction': 'column', gap: '20px', 'max-width': '760px' }}>
                  {/* Header */}
                  ...
                  <div style={{
                    ...
                    <DetailField label="Affected Assets" value={alert.affected_assets.join(', ') || '—'}/>
                    <DetailField label="Affected Users" value={alert.affected_users.join(', ') || '—'}/>
                    <DetailField label="Created" value={new Date(alert.created_at).toLocaleString('en-IN')}/>
                  ...
                  <Show when={alert.status === 'open' || alert.status === 'acknowledged'}>
                    <div>
                      <label for="resolution-notes" style={{ ...detailLabelStyle, display: 'block', 'margin-bottom': '6px' }}>
                        Resolution Notes (optional)
                      </label>
                      <textarea id="resolution-notes" rows={3} value={resolutionNotes()} onInput={(e) => setResolutionNotes(e.currentTarget.value)} placeholder="Describe what you found and what action was taken…" style={{
                    width: '100%',
                    padding: '9px 12px',
                    background: 'var(--surface)',
                    border: '1px solid var(--border)',
                  ...
                  <Show when={alert.status === 'open'}>
                    <div style={{ display: 'flex', gap: '8px', 'flex-wrap': 'wrap' }}>
                      <ActionButton label="Acknowledge" color="var(--primary)" onClick={() => void handleAcknowledge()}/>
                      <ActionButton label="Resolve" color="var(--success)" onClick={() => void handleResolve()}/>
                      <ActionButton label="False Positive" color="var(--text-muted)" onClick={() => void handleFalsePositive()}/>
                    </div>
                  </Show>
