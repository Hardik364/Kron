...
export default function Compliance() {
  const [reports, setReports] = createSignal<ReportRow[]>([]);
  ...
  async function loadReports() {
    setLoading(true);
    try {
      const data = await apiClient.getComplianceReports<ReportRow[]>();
      setReports(data);
    } catch (e) {
  ...
  async function downloadEvidence(reportId: string) {
    try {
  ...
  return (
    <div style={{ padding: '24px', 'max-width': '1000px' }}>
      ...
      <form
        ...
        style={{ display: 'flex', gap: '12px', 'align-items': 'flex-end', 'margin-bottom': '28px', 'flex-wrap': 'wrap' }}
      >
        <div>
        ...
        <button
          type="submit"
          disabled={generating()}
          style={{ padding: '8px 20px', 'border-radius': '6px', border: 'none', background: 'var(--accent)', color: '#fff', cursor: 'pointer', 'font-weight': '600', 'font-size': '14px', opacity: generating() ? '0.7' : '1' }}
        >
      ...
      <Show
        when={!loading()}
        fallback={<p style={{ color: 'var(--text-muted)' }}>Loading…</p>}
      >
        <Show
          when={reports().length > 0}
          fallback={<p style={{ color: 'var(--text-muted)', 'font-size': '14px' }}>No reports yet. Generate your first report above.</p>}
        >
          <table style={{ width: '100%', 'border-collapse': 'collapse' }}>
            <thead>
              <tr>
                {(['Framework', 'Period', 'Requested', 'Status', 'Actions'] as const).map((h) => (
                  <th style={{ 'text-align': 'left', padding: '8px 12px', 'font-size': '12px', 'font-weight': '600', color: 'var(--text-muted)', 'border-bottom': '1px solid var(--border)', 'text-transform': 'uppercase' }}>
                    {h}
                  </th>
                ))}
              </tr>
                  ...
                  <tr style={{ 'border-bottom': '1px solid var(--border)' }}>
                    ...
                    <td style={{ padding: '10px 12px' }}>
                      <Show when={r.status === 'ready'}>
                        <button
                          onClick={() => void downloadEvidence(r.report_id)}
                          style={{ padding: '4px 10px', 'border-radius': '4px', border: '1px solid var(--border)', background: 'transparent', color: 'var(--accent)', cursor: 'pointer', 'font-size': '12px', 'font-weight': '600' }}
                        >
                          ↓ Evidence ZIP
                        </button>
                      </Show>
