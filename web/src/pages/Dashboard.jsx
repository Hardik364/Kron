...
export default function DashboardPage() {
    ...
    return (<Layout title="Dashboard">
      ...
      <div style={{
        ...
        <MetricCard title="Open Alerts" value={state.isLoading ? '—' : openCount()} color="var(--primary)"/>
        <MetricCard title="P1 Critical" value={state.isLoading ? '—' : p1Count()} color="var(--critical)" subtitle="Requires immediate action"/>
        <MetricCard title="P2 High" value={state.isLoading ? '—' : p2Count()} color="var(--warning)" subtitle="Investigate within 1 h"/>
        <MetricCard title="Detection Engine" value="Online" color="var(--success)" subtitle="Stream processor active"/>
      </div>
      ...
      <div style={{
        ...
        <Show when={!state.isLoading} fallback={<div style={{ padding: '20px', display: 'flex', 'flex-direction': 'column', gap: '10px' }}>
          ...
          <Show when={state.error === null} fallback={<div style={{ padding: '20px', color: 'var(--danger)', 'font-size': '13px' }}>
                {state.error}
              </div>}>
            <table style={{ width: '100%', 'border-collapse': 'collapse' }} aria-label="Recent open alerts">
              <thead>
                <tr>
              ...
              <tbody>
                <Show when={recentAlerts().length > 0} fallback={<tr>
                      <td colspan={6} style={{
                ...tdStyle,
                'text-align': 'center',
                color: 'var(--text-muted)',
                padding: '32px',
                  ...
                  <For each={recentAlerts()}>
                    {(alert) => (<tr class="kron-dashboard-row">
                        <td style={tdStyle}>
                ...
                'white-space': 'nowrap',
                          ...
                          <A href={`/alerts`} style={{ color: 'var(--text)', 'text-decoration': 'none' }}>
                            {alert.rule_name}
                          </A>
                        </td>
