...
const SOURCE_TYPES = ['', 'windows', 'linux', 'network', 'cloud', 'ot'];
...
export default function EventsPage() {
    ...
    return (<Layout title="Event Search">
      {/* Search form */}
      <form onSubmit={handleSearch} style={{
        ...
        <div style={{ flex: '1 1 280px', 'min-width': '200px' }}>
          ...
          <input id="event-query" type="text" value={query()} onInput={(e) => setQuery(e.currentTarget.value)} placeholder="hostname, IP address, username, process…" style={{ ...inputStyle, width: '100%' }} aria-label="Event free-text search"/>
        </div>

        {/* From timestamp */}
        <div>
          <label for="event-from" style={{
            display: 'block',
            'font-size': '11px',
            color: 'var(--text-muted)',
            'margin-bottom': '4px',
          ...
          <input id="event-from" type="datetime-local" value={fromTs()} onInput={(e) => setFromTs(e.currentTarget.value)} style={{ ...inputStyle, 'font-family': 'var(--font-mono)', 'font-size': '12px' }}/>
        </div>

        {/* Source type filter */}
        <div>
          <label for="event-source" style={{
            display: 'block',
            'font-size': '11px',
            color: 'var(--text-muted)',
            'margin-bottom': '4px',
          ...
          <select id="event-source" value={sourceType()} onChange={(e) => setSourceType(e.currentTarget.value)} style={selectStyle}>
            <For each={SOURCE_TYPES}>
              {(s) => (<option value={s} style={{ background: 'var(--surface)' }}>
                  {s === '' ? 'All sources' : s}
                </option>)}
            </For>
          </select>
        ...
        {/* Severity filter */}
        <div>
          <label for="event-severity" style={{
            display: 'block',
            'font-size': '11px',
            color: 'var(--text-muted)',
            'margin-bottom': '4px',
          ...
          <select id="event-severity" value={severity()} onChange={(e) => setSeverity(e.currentTarget.value)} style={selectStyle}>
            <For each={SEVERITIES}>
              {(s) => (<option value={s} style={{ background: 'var(--surface)' }}>
                  {s === '' ? 'All severities' : s}
                </option>)}
        ...
        <label style={{
          ...
          <input type="checkbox" checked={iocOnly()} onChange={(e) => setIocOnly(e.currentTarget.checked)} style={{ cursor: 'pointer' }}/>
          IOC hits only
        </label>

        <button type="submit" disabled={state.isLoading} style={{
            padding: '8px 20px',
      ...
      <div style={{
        ...
        <div style={{ 'overflow-x': 'auto' }}>
          <table style={{ width: '100%', 'border-collapse': 'collapse' }} aria-label="Event search results">
            <thead>
              <tr>
            ...
            <tbody>
              <Show when={!state.isLoading} fallback={<For each={Array.from({ length: 12 })}>
                    {() => (<tr>
                ...
                <Show when={state.events.length > 0} fallback={<tr>
                      <td colspan={9} style={{
                ...tdStyle,
                'text-align': 'center',
                color: 'var(--text-muted)',
            ...
            return (<tr class="kron-event-row" style={{ background: rowColor }}>
                          <td style={{
                    ...tdStyle,
                    ...
                    'font-weight': iocHit ? '600' : 'normal',
                            ...
                            {event.dst_ip ?? '—'}
                            {iocHit && event.ioc_type && (<span style={{
                        'margin-left': '6px',
                        'font-size': '10px',
        ...
        <Show when={hasMore() && !state.isLoading && state.events.length > 0}>
          <div style={{ padding: '12px', 'border-top': '1px solid var(--border)', 'text-align': 'center' }}>
            <button onClick={() => void loadNextPage()} style={{
            padding: '7px 20px',
