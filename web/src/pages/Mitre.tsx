...
function heatColor(ratio: number): { background: string; border: string; text: string } {
  if (ratio === 0) {
    return {
 ...
 */
export default function MitrePage(): JSX.Element {
  ...
  const countForTactic = (tactic: MitreTactic): number =>
    state.alerts.filter(
      (a) =>
        a.mitre_tactic !== undefined &&
  ...
  const handleTacticClick = (tactic: MitreTactic): void => {
  ...
  return (
    <Layout title="MITRE ATT&CK Coverage">
      ...
      <div
        ...
        role="list"
        aria-label="MITRE ATT&CK tactic heatmap"
      >
        <For each={TACTICS}>
          {(tactic) => {
            ...
            return (
              <button
                ...
                aria-label={`${tactic.name}: ${count()} alerts. Click to filter.`}
                style={{
