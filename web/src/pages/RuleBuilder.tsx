...
type RuleMode = 'filter' | 'threshold';

export default function RuleBuilderPage(): ReturnType<Component> {
  const [name, setName] = createSignal('');
  const [severity, setSeverity] = createSignal<string>('P3');
  const [mitreTactic, setMitreTactic] = createSignal('');
  const [mode, setMode] = createSignal<RuleMode>('filter');
  ...
  const preview = () => {
    ...
    if (mode() === 'threshold') {
      lines.push(
        `  condition: selection | count(${field()}) by ${groupBy()} > ${thresholdCount()}`,
        `timeframe: ${windowSeconds()}s`,
      );
    } else {
  ...
  const handleSubmit = async (e: Event) => {
    ...
    try {
      ...
      // Reset form
      setName(''); setValue(''); setMitreTactic('');
      setSeverity('P3'); setMode('filter');
    } catch (err) {
        ...
        <div>
          <form onSubmit={(e) => void handleSubmit(e)}>
            ...
            <Field label="Detection Type">
              <div style={{ display: 'flex', gap: '8px' }}>
                {(['filter', 'threshold'] as RuleMode[]).map((m) => (
                  <button
                    type="button"
                    onClick={() => setMode(m)}
                    style={{
                ...
                <div style={{ display: 'flex', gap: '12px', 'align-items': 'center' }}>
                  <div>
                    ...
                    <input
                      ...
                      min={1}
                      onInput={(e) => setThresholdCount(Number(e.currentTarget.value))}
...
const feedbackStyle = (color: string) => ({
  padding: '10px 14px',
