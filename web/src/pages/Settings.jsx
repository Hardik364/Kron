...
function FieldGroup(props) {
    return (<div>
      ...
      <input id={props.id} type={props.type ?? 'text'} value={props.value} onInput={(e) => props.onInput(e.currentTarget.value)} placeholder={props.placeholder} style={inputStyle}/>
...
export default function SettingsPage() {
    ...
    const [saveMessage, setSaveMessage] = createSignal(null);
    /**
          ...
          <div style={{ display: 'flex', 'flex-direction': 'column', gap: '20px' }}>
            ...
            <div style={cardStyle}>
              ...
              <div style={{ display: 'flex', 'flex-direction': 'column', gap: '16px' }}>
                <FieldGroup id="org-name" label="Organisation Name" value={orgName()} onInput={setOrgName} placeholder="Acme Corp"/>
                <div style={{
            'font-size': '12px',
            color: 'var(--text-muted)',
            background: 'var(--surface)',
            ...
            <div style={cardStyle}>
              ...
              <div style={{ display: 'flex', 'flex-direction': 'column', gap: '16px' }}>
                <FieldGroup id="whatsapp-number" label="WhatsApp Number (P1 / P2 alerts)" type="tel" value={whatsappNumber()} onInput={setWhatsappNumber} placeholder="+91 98765 43210" hint="International format. P1 alerts are sent immediately; P2 alerts are batched every 15 min."/>
                <FieldGroup id="sms-number" label="SMS Number (P1 alerts only)" type="tel" value={smsNumber()} onInput={setSmsNumber} placeholder="+91 98765 43210" hint="SMS is used as fallback when WhatsApp delivery fails for P1 alerts."/>
                <FieldGroup id="alert-email" label="Alert Email" type="email" value={alertEmail()} onInput={setAlertEmail} placeholder="soc@organisation.in" hint="P1–P3 alert summaries are emailed at the end of each 1-hour window."/>
              </div>
            </div>
            ...
            gap: '16px',
                ...
                <FieldGroup id="p1-threshold" label="P1 Critical Threshold (0–100)" type="number" value={p1Threshold()} onInput={setP1Threshold} placeholder="80" hint="Risk score ≥ this value triggers a P1 alert."/>
                <FieldGroup id="p2-threshold" label="P2 High Threshold (0–100)" type="number" value={p2Threshold()} onInput={setP2Threshold} placeholder="60" hint="Risk score ≥ this value triggers a P2 alert."/>
              </div>
            </div>
            ...
            'border-radius': 'var(--radius-md)',
            'font-size': '13px',
            background: saveMessage()?.type === 'ok'
                ? 'rgba(16,185,129,0.1)'
                : 'rgba(239,68,68,0.1)',
