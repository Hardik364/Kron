...
export default function Tenants() {
    const [tenants, setTenants] = createSignal([]);
    ...
    return (<div style={{ padding: '24px', 'max-width': '1100px' }}>
      {/* Header */}
      ...
      {/* Tenant table */}
      <Show when={!loading()} fallback={<div style={{ color: 'var(--text-muted)', 'text-align': 'center', padding: '48px' }}>
            Loading tenants…
          </div>}>
        <Show when={tenants().length > 0} fallback={<div style={{ 'text-align': 'center', padding: '48px', color: 'var(--text-muted)' }}>
              No tenants yet. Create your first tenant to get started.
            </div>}>
          <table style={{ width: '100%', 'border-collapse': 'collapse' }}>
            <thead>
              <tr>
                {['Organisation', 'Contact', 'Frameworks', 'Language', 'Created', 'Status'].map((h) => (<th style={{ 'text-align': 'left', padding: '10px 12px', 'font-size': '12px', 'font-weight': '600', color: 'var(--text-muted)', 'border-bottom': '1px solid var(--border)', 'text-transform': 'uppercase', 'letter-spacing': '0.05em' }}>
                    {h}
                  </th>))}
              </tr>
            </thead>
            <tbody>
              <For each={tenants()}>
                {(tenant) => (<tr style={{ 'border-bottom': '1px solid var(--border)' }}>
                    ...
                    <td style={{ padding: '12px' }}>
                      <div style={{ display: 'flex', gap: '4px', 'flex-wrap': 'wrap' }}>
                        <For each={tenant.compliance_frameworks}>
                          {(fw) => (<span style={{ background: 'var(--surface2)', padding: '2px 6px', 'border-radius': '4px', 'font-size': '11px', 'font-weight': '600' }}>
      ...
      <Show when={showCreate()}>
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', 'align-items': 'center', 'justify-content': 'center', 'z-index': '100' }} onClick={(e) => { if (e.target === e.currentTarget)
        setShowCreate(false); }}>
          <div style={{ background: 'var(--surface)', padding: '32px', 'border-radius': '12px', width: '480px', 'max-width': '90vw' }}>
            ...
            <form onSubmit={(e) => void handleCreate(e)}>
              <Field label="Organisation Name *">
                <input type="text" value={form().name} onInput={(e) => setForm((f) => ({ ...f, name: e.currentTarget.value }))} placeholder="Acme Corp Pvt. Ltd." style={inputStyle}/>
              </Field>

              <Field label="Contact Email *">
                <input type="email" value={form().contact_email} onInput={(e) => setForm((f) => ({ ...f, contact_email: e.currentTarget.value }))} placeholder="ciso@acme.example" style={inputStyle}/>
              </Field>

              <Field label="WhatsApp Number (E.164)">
                <input type="text" value={form().whatsapp_number} onInput={(e) => setForm((f) => ({ ...f, whatsapp_number: e.currentTarget.value }))} placeholder="+919876543210" style={inputStyle}/>
              </Field>
            ...
            return (<button type="button" onClick={() => toggleFramework(fw.id)} style={{
                    padding: '4px 10px',
                    'border-radius': '4px',
              ...
              <Field label="Language">
                <select value={form().language} onChange={(e) => setForm((f) => ({ ...f, language: e.currentTarget.value }))} style={inputStyle}>
                  <For each={LANGUAGES}>
              ...
              <div style={{ display: 'flex', gap: '12px', 'margin-top': '24px', 'justify-content': 'flex-end' }}>
                <button type="button" onClick={() => setShowCreate(false)} style={{ padding: '8px 16px', 'border-radius': '6px', border: '1px solid var(--border)', background: 'transparent', cursor: 'pointer', 'font-size': '14px' }}>
                  Cancel
                </button>
                <button type="submit" disabled={creating()} style={{ padding: '8px 20px', 'border-radius': '6px', border: 'none', background: 'var(--accent)', color: '#fff', cursor: 'pointer', 'font-size': '14px', 'font-weight': '600', opacity: creating() ? '0.7' : '1' }}>
...
function parseStyle(css) {
    return Object.fromEntries(css.split(';').filter(Boolean).map((rule) => {
