...
export function useAlerts() {
    ...
    const acknowledgeAlert = async (id) => {
        try {
            ...
            setState('alerts', (alerts) => alerts.map((a) => (a.alert_id === id ? { ...a, status: 'acknowledged' } : a)));
        }
        catch (err) {
     ...
     */
    const markFalsePositive = async (id, notes) => {
        try {
            await apiClient.markFalsePositive(id, notes);
            setState('alerts', (alerts) => alerts.map((a) => a.alert_id === id ? { ...a, status: 'false_positive' } : a));
        }
        catch (err) {
