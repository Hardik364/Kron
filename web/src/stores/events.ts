...
import type { EventQuery, KronEvent } from '../types/api';
...
export function useEvents() {
   ...
   */
  const searchEvents = async (params: EventQuery): Promise<void> => {
  ...
  const loadNextPage = async (): Promise<void> => {
    ...
    const nextQuery: EventQuery = { ...state.currentQuery, offset: currentOffset + limit };
